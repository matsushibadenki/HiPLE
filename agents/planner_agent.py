...
"""
        ...
        judgement_criteria = """
# 判断基準 (最重要)
1.  **抽象的なタスク記述**: タスクの`description`および`ssv_description`には、「ツール `web_search` を使って〜」のような具体的な実行方法を記述せず、「アイスクリームの作り方を調査する」のように、**何をすべきかという目的**のみを記述してください。担当エキスパートが実行時に最適なツールを自律的に判断します。
2.  **情報検索タスクの計画**: ユーザーの要求が外部情報の調査を必要とする場合、2段階の計画を立ててください。
    - **ステップ1 (調査タスク)**: 情報を調査するタスクを作成します。エキスパートは `Jamba` を指定してください。
    - **ステップ2 (加工タスク)**: ステップ1の結果を要約・抽出し、最終的な回答を作成するタスクを作成します。`dependencies` でステップ1に依存させてください。
        ...
        "tasks": [
            {
                "task_id": 1,
                "milestone_id": 1,
                "description": "インターネットで「アイスクリーム 簡単 レシピ」について調査する",
                "expert_name": "Jamba",
