...
class WikipediaAgent(BaseAgent):
    ...
    def execute(self, query: str, all_experts: List[ExpertModel]) -> str:
        ...
        system_prompt = "あなたは、与えられたWikipediaの記事の抜粋を分析し、ユーザーの元の質問に答える形で、最も重要なポイントを抽出して分かりやすく再構成する優秀な編集者です。"
...
# あなたのタスク
上記の抜粋から、ユーザーの質問に答えるために必要な情報だけを抽出し、箇条書きなどを用いて分かりやすい最終的な回答を生成してください。
"""
