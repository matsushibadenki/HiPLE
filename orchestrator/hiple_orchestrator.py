    ...
    def _execute_plan(self, plan: Plan, experts: List[ExpertModel]) -> str:
        ...
        while len(completed_tasks) < len(worker_tasks):
            ...
            for task in executable_tasks:
                ...
                # ツール利用の再試行ループは不要なため、フィードバックループのみに修正
                for loop_count in range(self.max_feedback_loops + 1):
                    ...
                    if response_dict.get("status") == "tool_request":
                        ...
                        self.workspace.add_thought("orchestrator", "tool_result_as_outcome", f"Task {task.task_id} completed with tool result.")
                        break
                ...
                self.workspace.add_thought(
                    "orchestrator", 
                    "task_completed", 
                    {"task_id": task.task_id, "status": task.status, "self_evaluation": task.self_evaluation, "error_content": task.result if not success else None}
                )
                
                if not success:
                    print(f"❌ タスク {task.task_id} が完了しました。ステータス: {task.status}\n  - エラー内容: {task.result}")
                    break
                else:
                    print(f"✅ タスク {task.task_id} が完了しました。ステータス: {task.status}\n  - 結果のプレビュー: {task.result[:100].replace(chr(10), ' ')}...")
